...
class DataCleaner:
    ...
    def write_cleaned_conversations(self, out_file, limit=None):
        """
        Args:
        ...
        with open(out_file, 'a') as f_out:
            for conversation in conversations:
                ...
                input_line = conversation[0].strip()
                target_line = conversation[1].strip()
                ...
                if all(ord(char) < 123 and ord(char) not in special_chars for char in input_line) and \
                    ...
                    f_out.write("Q: {}\n".format(' '.join(in_tokens)))
                    f_out.write("A: {}\n".format(' '.join(tg_tokens)))
                    f_out.write("===\n")
...
if __name__ == "__main__":
    corp_dir = os.path.join('.', 'Data', 'Corpus', 'OpenSubtitles', 'Data')
